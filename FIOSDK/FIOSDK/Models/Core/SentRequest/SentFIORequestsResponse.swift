    ...
    public struct SentFIORequestsResponse: Codable {
        ...
        public struct SentFIORequestResponse: Codable {
            ...
            public init(from decoder: Decoder) throws {
                ...
                var metadataString = FIOSDK.sharedInstance().decrypt(publicKey: payerFIOPublicKey, contentType: FIOAbiContentType.newFundsContent, encryptedContent: content)
                
                if (metadataString.count < 1){
                    metadataString = FIOSDK.sharedInstance().decrypt(publicKey: payeeFIOPublicKey, contentType: FIOAbiContentType.recordObtDataContent, encryptedContent: content)
                }

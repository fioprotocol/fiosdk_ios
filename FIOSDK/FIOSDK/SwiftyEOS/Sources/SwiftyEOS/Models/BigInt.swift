    ...
    where Word.Magnitude == Word
{
    ...
    public init<T : BinaryInteger>(_ source: T) {
        ...
        //xcode 10.2 fix _sanityCheck(wordRatio != 0)
    ...
    mutating func _unsignedSubtract(_ rhs: Word) {
        //xcode 10.2 fix _precondition(_data.count > 1 || _data[0] > rhs)
        if (_data.count > 1 || _data[0] > rhs){
            // Quick return if `rhs == 0`
            guard rhs != 0 else { return }
            
            // If `isZero == true`, then `rhs` must also be zero.
            //xcode 10.2 fix _precondition(!isZero)
            if (!isZero){
                var carry: Word
                ...
                //xcode 10.2 fix _sanityCheck(carry == 0)
    ...
    mutating func divide(by rhs: Word) -> Word {
        //xcode 10.2 fix _precondition(rhs != 0, "divide by zero")
        if (rhs != 0){
    ...
    mutating func _unsignedSubtract(_ rhs: _BigInt) {
        //xcode 10.2 fix _precondition(rhs._data.count <= _data.count)
        if (rhs._data.count <= _data.count){
            var carry: Word = 0
    ...
    public static func *=(lhs: inout _BigInt, rhs: _BigInt) {
        ...
        let (a, b) = lhs._data.count > rhs._data.count
            ? (lhs._data, rhs._data)
            : (rhs._data, lhs._data)
        //xcode 10.2 fix _sanityCheck(a.count >= b.count)
        ...
        for ai in 0..<a.count {
            ...
            for bi in 0..<b.count {
                ...
                //xcode 10.2 fix _sanityCheck(!product.high.addingReportingOverflow(carry).overflow)
                carry = product.high &+ carry
            }
            
            // Leftover `carry` is inserted in new highest word.
            //xcode 10.2 fix _sanityCheck(newData[ai + b.count] == 0)
    ...
    mutating func _internalDivide(by rhs: _BigInt) -> _BigInt {
        //xcode 10.2 fix _precondition(!rhs.isZero, "Divided by zero")
        if (!rhs.isZero){
            defer { _checkInvariants() }
    ...
    public var words: [UInt] {
        //xcode 10.2 fix _sanityCheck(UInt.bitWidth % Word.bitWidth == 0)
    ...
    public var trailingZeroBitCount: Int {
        ...
        //xcode 10.2 fix _sanityCheck(_data[i] != 0)

...
@objcMembers public class Account: NSObject, Codable {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        if let ramQuota = try? container.decodeIfPresent(Int64.self, forKey: .ramQuota) {
            self.ramQuota = ramQuota!
        } else if let ramQuota = try? container.decodeIfPresent(String.self, forKey: .ramQuota) {
            self.ramQuota = Int64(ramQuota!)!
        }
        
        if let ramUsage = try? container.decodeIfPresent(Int64.self, forKey: .ramUsage) {
            self.ramUsage = ramUsage!
        } else if let ramUsage = try? container.decodeIfPresent(String.self, forKey: .ramUsage) {
            self.ramUsage = Int64(ramUsage!)!
        }
...
@objcMembers class ResourceLimit: NSObject, Codable {
    ...
    init(used: Int64, available: Int64, max: Int64) {
        self.used = used
    ...
    required init(from decoder: Decoder) throws {
        ...
        if let used = try? container.decodeIfPresent(Int64.self, forKey: .used) {
            self.used = used!
        } else if let used = try container.decodeIfPresent(String.self, forKey: .used) {
            self.used = Int64(used)!
        } else {
        ...
        if let available = try? container.decodeIfPresent(Int64.self, forKey: .available) {
            self.available = available!
        } else if let available = try container.decodeIfPresent(String.self, forKey: .available) {
            self.available = Int64(available)!
        } else {
        ...
        if let max = try? container.decodeIfPresent(Int64.self, forKey: .max) {
            self.max = max!

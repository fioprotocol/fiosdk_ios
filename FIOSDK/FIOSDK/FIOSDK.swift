...
public class FIOSDK: NSObject {
    ...
    private func register(fioName:String, completion: @escaping ( _ error:FIOError?) -> ()) {
        ...
        #warning("make this call more generic, reusable for the other signed transactions")  ///TODO:  make "/chain/serialize_json" an ENUM, so it can be changed in a central place.
        let url = getURI() + "/chain/serialize_json"  /// TODO: break this out into it's own method, so we can reuse this for the next call
            ...
            if let data = data {
                do {
                    ...
                    TransactionUtil.packAndSignTransaction(code: "fio.system", action: "registername", data: result.json, account: actor, privateKey: importedPk, completion: { (signedTx, error) in
                        if error != nil {   ///TODO: move this error logic into a re-usable method
                            ...
                            print("Called FIOSDK action registername")   ///TODO: make /chain/register_fio_name part of an ENUM, so it can be changed in a central place.
